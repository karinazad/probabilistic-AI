...
class BO_algo(object):
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        if self.new:
            ...
            f_kernel = ConstantKernel(constant_value=1.5, constant_value_bounds="fixed") * RBF(length_scale=1.5,
                                                                                               length_scale_bounds="fixed") + WhiteKernel(
                noise_level=0.01 ** 2)
            ...
            c_kernel = ConstantKernel(constant_value=3.5, constant_value_bounds="fixed") * RBF(length_scale=2,
                                                                                               length_scale_bounds="fixed") + WhiteKernel(
                noise_level=0.005 ** 2)
...
def perform_extended_evaluation(agent, output_dir='./'):
    ...
    im_cond = ax_condition.pcolormesh(xv, yv, conds.reshape((EVALUATION_GRID_POINTS, EVALUATION_GRID_POINTS)),
                                      shading='auto',
                                      linewidth=0)
    ...
    im_predictions = ax_predictions.pcolormesh(xv, yv, predictions, shading='auto', label='Posterior', linewidth=0,
                                               antialiased=True)
